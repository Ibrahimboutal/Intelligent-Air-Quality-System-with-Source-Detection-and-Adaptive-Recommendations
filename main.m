...
% Add the source directory to the MATLAB path
    ...
    Set this to false when you have your Raspberry Pi and SDS011 connected
        simulationMode = true;
useKalman = true;
% Enable Bayesian Signal Denoising(Phase 1)

    % Load credentials from.env file securely loadEnv('.env');
...
baud_rate = str2double(baud_str);
    ...
    % %
    2. Initialize System disp('Initializing Intelligent Air Quality System...');
aqSystem = AirQualitySystem(pi_ip, pi_user, pi_pass, serial_port, baud_rate,
                            simulationMode, useKalman);

% Safety : Guarantee hardware release on interrupt(Ctrl + C)
               cleanupHardware = onCleanup(@() delete (aqSystem));

% Connect to the hardware(or setup simulation) aqSystem.connect();

% % 3. Start Monitoring num_samples = 1500;
% Number of seconds to run aqSystem.run(num_samples);

% % 4. Advanced Robust Analysis(FSDA) % Performs post
    - processing on the collected data to find hidden anomalies.aqSystem
          .runFSDAAnalysis();
